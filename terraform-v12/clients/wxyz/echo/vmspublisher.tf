...
module "ec2-publisher" {
  ...
  instance_type                                    = var.instance_type_publisher
  key_name                                         = var.key_name
  ##(default) get_password_data                    = false
  ##(default) monitoring                           = false
  vpc_security_group_ids                           = [module.secgrp-publisher.security_group_id]
...
module "eip-publisher" {
  source                                  = "../../../modules/aws_eip"
  ##(default) vpc                         = true
  instance_id                             = module.ec2-publisher.id
}
...
module "route53-publisher" {
  source                                  = "../../../modules/aws_route53_record"
  providers                               = {aws = aws.veolia}
  ##(default) zone_id                     = "Z1Q9CG9D_notreal"
  name                                    = "${var.client_name}-${var.environment_name}-${var.p_tier}-${var.application_version}-${var.region_code}"
  type                                    = "A"
  records                                 = module.eip-publisher.public_ip
  ##(default) ttl                         = "120"
}

#--------------------------------------------------------------------------------------
# chef provision ##wxyz-echo-publisher-65-usw2c
#--------------------------------------------------------------------------------------
module "chef-provision-publisher" {
  source                                  = "../../../modules/chef_provision_linux"
  ##(default) type                        = "ssh"
  user                                    = var.ssh_user
  host                                    = module.eip-publisher.public_ip
  ##(default) port                        = "22"
  ##(default) timeout                     = "10m"
  ##(default) script_path                 = ""
  ...
  node_name                               = "${var.client_name}-${var.environment_name}-${var.p_tier}-${var.application_version}-${var.region_code}"
  ##(default) ohai_hints                  = ""
  ##(default) os_type                     = "linux"
  ##(default) recreate_client             = true
  run_list                                = var.run_list_publisher
